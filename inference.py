...
def evaluate_model(bearded_dir, clean_dir, checkpoint_path, num_examples=20, device="cuda"):
    ...
    for img_name in bearded_images:
        ...
        output_np = np.array(transforms.ToPILImage()(denormalize(output_tensor.squeeze(0).cpu())))
        clean_np = np.array(clean_img)
        
        ssim_val = ssim(clean_np, output_np, multichannel=True)
        ...
        lpips_val = loss_fn_alex(output_tensor, clean_tensor)
